...
class environment:
    ...
    def create_action_space(self,all_vals=False,state=None):
        action_space = []
        if state is None:
            state = self.state
        if np.shape(state)[0]<self.max_number_nodes or all_vals==True:
    ...
    def determine_reward(self,state,features,workflow_vals):
        ...
        graph_truth = environment.create_DAG(self.truth_state,self.truth_features)
