...
from agents.Replay import ExperienceReplay
...
class RL2:
    ...
    def epsilon_greedy(self):
        ...
        for e in range(self.episode_length):
            ...
            for (state,features,reward,done) in self.memory.sample(5):
                state_tuple = RL2.get_state(state, features)
                q_val = self.compute_Q(state_tuple)
                step_q = self.bellman_q_replay(reward,state,features,done)
            ...
            state_tuple = RL2.get_state(self.env.state,self.env.features)
            q_val = self.compute_Q(state_tuple)
    ...
    def best_action_replay(self,state,features):
        self.TargetNetwork.eval()
        max_q = -float('Inf')
        actions_list = self.env.create_action_space(state=state)
        random.shuffle(actions_list)
        best_action = actions_list[0]
        amount_to_sample = len(actions_list) if self.max_sample_size == 0 else min(
            self.max_sample_size, len(actions_list))
        for i in range(amount_to_sample):
            # search whole action space or to max sample search whichever is smaller
            action = actions_list[i]
            new_state, new_features = self.env.take_action(state, features, action)
            state_tuple = RL2.get_state(new_state, new_features)
            q = self.compute_Q(state_tuple, self.TargetNetwork)  # what is the value of this state
            if q > max_q:
                max_q = q
                best_action = action
    ...
    def bellman_q_replay(self,reward,state,features,done):
        r = reward
        if done:
            q_max = 0
        else:
            _, q_max = self.best_action_replay(state,features)
